...
| `brain <brain-topic>` | 新建 session，并按 `/brain` 收敛后写入任务、计划摘要和工作集 | `/session new` → `/brain` → 内联 `/plan` 语义，结果直接写入 session |
...
等价于：`/session new <brain-topic>` → `/brain <brain-topic>` → 内联 `/plan` 的产出规则，写入 session 的任务列表、计划摘要和当前批次工作集。

额外约束：
- `/brain` 的澄清、方案对比、推荐、默认假设规则，统一以 `.shared/commands/brain.md` 为准，`/session brain` 不再重复定义一套流程
- session 模式只沉淀最终确认结论、任务列表、计划摘要和必要的当前批次工作集；方案对比默认保留在当前回复，不默认落 `.tmp/agentwork/brain/*.md` 或 `.tmp/agentwork/plan/*.md`
