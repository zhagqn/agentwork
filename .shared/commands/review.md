...
5. 压缩历史记录：压缩重复路径、命令流水和重复验证，不按固定条数删除有独立语义的提交锚点、决策、约束、证据或风险。
   ...
   - 只有 session 文件自身更新、格式/生成物同步或没有新增语义的连续提交，才合并为 `历史:` 阶段摘要；摘要保留时间范围和可追溯提交锚点（若存在）。
   ...
   - 已关闭或仅作归档指针的 session 可以主要保留历史摘要，不强行补造近期详情；前提是目标、最终结论、验证边界、未闭环风险和接续 session / 提交锚点仍可追溯。
6. 完成前自检：确认 session 仍是可恢复任务快照，并完成“信息保留检查”，而不是只得到结构通过。
...
- 当前批次工作集压缩：小批次可列精确路径；跨目录或超过约 20 个路径时改写为主题/目录范围，精确路径以 `git status`、`git diff`、`.shared/scripts/session-review.sh` 取证，不长期堆入 session。工作集压缩不得替代产出批次中的提交锚点。
...
- 审查记录压缩：先解析标题日期并按时间倒序整理，不按原文位置推断新旧；保留最近 3 个有独立结论/验证的审查事件，`历史审查摘要`不计入名额，更早事件合并为按里程碑/时间段的一句话或简述。旧 session 或归档指针可直接保留历史摘要，但必须保留结论、验证状态、未闭环风险和来源锚点。不记录命令流水、重复验证过程、临时判断或已被 project 文档吸收的长期事实。
- 信息保留检查：压缩前逐项判断内容属于当前事实、关键决策/取舍、验证证据、兼容/迁移前提、风险/回滚边界、重复流水或已迁移内容。前五类必须保留在 session 或明确指向另一个 session、project、commit 或 review 工件；只有后两类可以删除或合并。
- 提交记录格式：`提交:` 后可列多个相关 hash；每条仍需有 `范围:` 和 `验证:`，范围按主题/目录归类，不要求罗列所有文件。
- 提交锚点可验证性：当前仓找不到的 hash 只能标记为“当前仓不可验证”，不得仅因 `session-review.sh` 无法校验就删除；需确认它是否来自旧分支、历史重写、独立仓或尚未同步的对象。
