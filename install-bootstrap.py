...
def strip_managed_codex_config_block(text: str) -> tuple[str, bool]:
    start_count = text.count(CODEX_CONFIG_BLOCK_START)
    end_count = text.count(CODEX_CONFIG_BLOCK_END)
    if start_count == 0 and end_count == 0:
        return text, False
    if start_count != 1 or end_count != 1:
        raise SystemExit('Codex config has a malformed or duplicate agentwork managed block')
    start_idx = text.find(CODEX_CONFIG_BLOCK_START)
    end_idx = text.find(CODEX_CONFIG_BLOCK_END)
    if start_idx > end_idx:
        raise SystemExit('Codex config has an invalid agentwork managed block')
    end_idx += len(CODEX_CONFIG_BLOCK_END)
...
def preflight_codex_agent_writes(target: Path, writes) -> None:
    marker = CODEX_AGENT_MARKER.encode('utf-8')
    for src, dst, _, label in writes:
        if label != 'Codex agent' or src.resolve() == dst.resolve():
            continue
        reject_symlink_path(dst, target, 'Codex agent')
        if not dst.exists():
            continue
        if not dst.is_file():
            raise SystemExit(f'cannot replace custom Codex agent path: {dst}')
        existing = dst.read_bytes()
        if not existing.startswith(marker) and existing != src.read_bytes():
            raise SystemExit(f'cannot replace non-agentwork Codex agent file: {dst}')


def write_codex_config(config: Path, content: bytes) -> None:
...
def main() -> int:
    ...
    preflight_codex_agent_writes(target, writes)
    codex_config, codex_config_content = prepare_codex_config(target)
    ...
    write_codex_config(codex_config, codex_config_content)
