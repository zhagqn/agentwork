...
RETIRED_CODEX_AGENT = Path('.codex/agents/luna-worker.toml')
RETIRED_CODEX_AGENT_SHA256 = 'd2dd50b5e303be8b199da8680544ce32408540cf7691976848fb13d59a53d14b'
# 旧版默认注册块的完整摘要；改写过的受管块也保留，避免删除用户配置。
RETIRED_CODEX_BLOCK_SHA256 = 'a3a3c75f7dc0e875bff37e6adf953aca0553acc31ef5e64bb373e6cb57a8ca3e'
...
def prepare_codex_config(target: Path) -> PreparedFile | None:
    ...
    span = managed_block_span(original, CODEX_CONFIG_BLOCK_START, CODEX_CONFIG_BLOCK_END, 'Codex config')
    if span is None:
        return None
    start, end = span
    block = original[start:end].replace('\r\n', '\n') + '\n'
    if sha256_bytes(block.encode('utf-8')) != RETIRED_CODEX_BLOCK_SHA256:
        return None
    # 仅删除已知旧块及其自身行尾；块外字节保持不变。
    if original[end:].startswith('\r\n'):
        end += 2
    elif original[end:].startswith('\n'):
        end += 1
    ...
    try:
        ...
        raise SystemExit(f'cannot retire Codex config block: {exc}') from exc
    # 文本相同不代表是配置块：可能位于多行字符串内，或影响后续表的归属。
    agents = before.get('agents')
    role = tomllib.loads(block)['agents']['luna_worker']
    if not isinstance(agents, dict) or agents.get('luna_worker') != role:
        return None
    del agents['luna_worker']
    if not agents:
        del before['agents']
    if after.get('agents') == {}:
        del after['agents']
    if before != after:
        return None
    return PreparedFile(config, updated.encode('utf-8'))
...
def prepare_bootstrap_plan(
    ...
    *,
    ...
    if codex_config is None and config_path.is_file() and CODEX_CONFIG_BLOCK_START.encode() in config_path.read_bytes():
        retired_preserved += (Path('.codex/config.toml'),)
    if RETIRED_CODEX_AGENT in retired_removed and config_path.is_file():
        content = codex_config.content if codex_config else config_path.read_bytes()
        # 配置仍引用旧文件时保留它；解析失败也不猜测删除。
        try:
            parsed = tomllib.loads(content.decode('utf-8'))
            agents = parsed.get('agents', {})
            referenced = any(
                isinstance(role, dict) and isinstance(role.get('config_file'), str)
                and (config_path.parent / role['config_file']).resolve() == (target / RETIRED_CODEX_AGENT).resolve()
                for role in agents.values()
            ) if isinstance(agents, dict) else True
        except (UnicodeError, tomllib.TOMLDecodeError):
            referenced = True
        if referenced:
            retired_removed = tuple(rel for rel in retired_removed if rel != RETIRED_CODEX_AGENT)
            retired_preserved += (RETIRED_CODEX_AGENT,)
    if codex_config is not None and not codex_config.content.strip():
        retired_removed += (Path('.codex/config.toml'),)
        codex_config = None
    if target != ROOT:
    ...
    transaction_paths = collect_transaction_paths(
        ...
        [
            ...
            *([codex_config.path] if codex_config else []),
...
def apply_bootstrap_plan(plan: BootstrapPlan) -> None:
    ...
    if plan.codex_config is not None:
        ...
        print('- removed retired Codex agent registration block')
