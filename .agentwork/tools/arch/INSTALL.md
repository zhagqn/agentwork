...
- 若图源是 Mermaid，可先通过 `.shared/scripts/arch-export-mermaid.py docs/architecture` 刷新 `diagram.svg`
- renderer 负责生成根导航 `index.html`、item `index.html` 和 `assets/arch.css`
- 通过 `.shared/scripts/arch-render.py docs/architecture --check` 验证 catalog、source、SVG 与链接
- 通过 `.shared/scripts/arch-render.py docs/architecture` 生成静态页面
