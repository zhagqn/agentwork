...
LEGACY_CODEX_BLOCK = "# >>> AGENTWORK bootstrap: Codex agents >>>\n# AUTO-GENERATED by agentwork bootstrap\n[agents.luna_worker]\ndescription = \"首选执行型子代理：凡范围明确、边界清晰且可独立完成并验证的委派任务，优先使用本 Agent；不用于调整整体任务目标、跨任务协调或开放式扩展。\"\nconfig_file = \"agents/luna-worker.toml\"\n# <<< AGENTWORK bootstrap: Codex agents <<<\n".encode('utf-8')
...
class InstallBootstrapCodexAgentTest(unittest.TestCase):
    ...
    def test_fresh_install_has_no_managed_codex_agent(self) -> None:
        ...
        self.assertFalse((self.project / '.codex/agents/luna-worker.toml').exists())
    ...
    def test_reinstall_preserves_other_config_and_agents(self) -> None:
        ...
        self.assertNotIn('luna_worker', config['agents'])
        self.assertEqual(first_config, custom_config)
    ...
    def test_legacy_codex_agent_retirement_preserves_user_changes(self) -> None:
        module = self.load_installer()
        for variant in ('unchanged', 'edited-file', 'edited-block', 'external-reference', 'crlf'):
            with self.subTest(variant=variant):
                project = Path(self.temp.name) / variant
                agent = project / '.codex/agents/luna-worker.toml'
                agent.parent.mkdir(parents=True)
                original = b'# AUTO-GENERATED by agentwork bootstrap\nmodel = "legacy"\n'
                agent.write_bytes(original + (b'# custom\n' if variant == 'edited-file' else b''))
                config = project / '.codex/config.toml'
                block = LEGACY_CODEX_BLOCK
                if variant == 'edited-block':
                    block = block.replace(b'[agents.luna_worker]', b'[agents.custom]')
                if variant == 'crlf':
                    block = block.replace(b'\n', b'\r\n')
                suffix = b'\n[agents.reviewer]\nconfig_file = "agents/luna-worker.toml"\n' if variant == 'external-reference' else b''
                config.write_bytes(block + suffix)
                receipt = project / RECEIPT_REL
                receipt.parent.mkdir()
                receipt.write_text(json.dumps({'schema_version': 1, 'files': [{
                    'path': '.codex/agents/luna-worker.toml', 'type': 'file',
                    'sha256': hashlib.sha256(original).hexdigest(),
                }]}))
                before = tree_snapshot(project)
                plan = module.prepare_bootstrap_plan(project.resolve())
                with patch.object(module, 'write_receipt', side_effect=OSError('retirement failure')):
                    with self.assertRaises(SystemExit):
                        module.run_transaction(project.resolve(), plan.transaction_paths,
                                               lambda: module.apply_bootstrap_plan(plan))
                self.assertEqual(tree_snapshot(project), before)
                self.run_installer(project=project)
                self.run_installer(project=project)
                self.assertEqual(agent.exists(), variant in {'edited-file', 'edited-block', 'external-reference'})
                if variant == 'edited-block':
                    self.assertEqual(config.read_bytes(), block)
                elif variant == 'external-reference':
                    self.assertEqual(config.read_bytes(), suffix)
                else:
                    self.assertFalse(config.exists())

    def test_retirement_without_receipt_preserves_custom_generated_file(self) -> None:
    ...
    def test_retirement_preserves_block_embedded_in_toml_string(self) -> None:
        config = self.project / '.codex/config.toml'
        config.parent.mkdir()
        original = b'note = """\n' + LEGACY_CODEX_BLOCK + b'"""\n'
        config.write_bytes(original)
        self.run_installer()
        self.assertEqual(config.read_bytes(), original)

    def test_retirement_preserves_fields_following_managed_agent_table(self) -> None:
        config = self.project / '.codex/config.toml'
        config.parent.mkdir()
        original = LEGACY_CODEX_BLOCK + b'custom_field = "preserve"\n'
        config.write_bytes(original)
        self.run_installer()
        self.assertEqual(config.read_bytes(), original)

    def test_existing_custom_same_role_is_preserved(self) -> None:
        codex = self.project / '.codex'
        codex.mkdir()
        original = (
            '[agents.luna_worker]\n'
    ...
    def test_hardlinked_managed_config_is_replaced_without_external_write(self) -> None:
        ...
        self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
        self.assertEqual(external.read_bytes(), external_content)
        self.assertEqual(target.read_bytes(), b'model = "project-model"\n')
        self.assertNotEqual(target.stat().st_ino, external.stat().st_ino)
    ...
    def test_codex_config_failure_rolls_back_complete_tree(self) -> None:
        config = self.project / '.codex/config.toml'
        config.parent.mkdir()
        config.write_bytes(b'model = "project-model"\n' + LEGACY_CODEX_BLOCK)
        module = self.load_installer()
        plan = self.prepared_plan(module)
